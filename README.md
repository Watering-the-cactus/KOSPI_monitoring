...
코스피 전종목의 개인/기관/외국인/기타법인 순매수 데이터를 매일 장마감 이후 수집해서,
다음날 개장 전(기본 KST 07:30) 이메일로 발송하는 파이프라인입니다.

## 리포트 구성

1. **A. 외국인 연속 순매수 종목** — 최근 10거래일 중 N일 이상 외국인 순매수 종목
   (거래대금 변화율, 지수대비 초과등락률을 보조지표로 함께 표시)
2. **B. 추세 전환 관찰 종목** — 최근 3일 평균 순매수 부호가 이전 17일 평균과 반대인 종목
3. **C. 종목별 뉴스 요약/관전 포인트** — A/B에서 뽑힌 종목에 대해 뉴스 검색 + 요약
4. **D. 상세 대시보드** — GitHub Pages에 매일 게시되는 전종목 정렬/검색 가능한 표
5. **E. (부록) 투자자 유형별 순매수/순매도 상위 20종목** + 전종목 xlsx 첨부
