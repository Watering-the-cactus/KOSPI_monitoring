...
3. **🔄 추세 전환 관찰** — 최근 3일 평균 순매수 부호가 이전 17일 평균과 반대인 종목 (보조)
4. **📊 상세 대시보드** — GitHub Pages에 매일 게시되는 전종목 정렬/검색 가능한 표

투자자 유형별 순매수/순매도 상위 종목, 탑픽 전체 리스트는 이메일 본문 대신
**첨부 xlsx의 별도 시트**로만 정리한다 (`전종목`, `외국인연속매수_전체`,
